...
ssh -F "$SSH_CONFIG" jws "cd /opt && tar czf /tmp/staging-backup.tar.gz theoshift/"
...
cd /opt
rm -rf theoshift-old
mv theoshift theoshift-old 2>/dev/null || true
tar xzf /tmp/staging-backup.tar.gz
chown -R root:root theoshift/
...
find . -name '*.ts' -o -name '*.tsx' -o -name '*.js' -o -name '*.jsx' | grep -v node_modules | xargs grep -l 'blue.theoshift.com' 2>/dev/null | while read file; do
  echo \"Updating \$file\"
...
echo ''
echo 'SUCCESS CHECK:'
curl -s http://10.92.3.22:3001/api/auth/providers | grep 'theoshift.com' && echo '🎉 SUCCESS!' || echo '❌ Still staging URLs'
