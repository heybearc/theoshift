...
**Environment Details:**
- **Blue (Production):** 10.92.3.22:3001 (Container 132/133)
- **Green (Standby):** 10.92.3.24:3001 (Container 134/135)
...
npm install
npx prisma generate
pm2 stop jw-attendant-blue
rm -rf .next
npm run build
pm2 start jw-attendant-blue
...
npm install
npx prisma generate
pm2 stop jw-attendant-green
rm -rf .next
npm run build
pm2 start jw-attendant-green
```
...
```bash
# Via MCP server
mcp3_switch_traffic --app jw-attendant --emergency true
...
```bash
pm2 status
pm2 logs jw-attendant-[blue|green]
pm2 monit
```
